<template>
  <div class="min-h-screen bg-white dark:bg-gray-900 text-gray-900 dark:text-gray-100" :class="{'dark': isDarkMode}">
    ...
    <div class="border-b border-gray-200 dark:border-gray-700 px-6 py-4 sticky top-0 bg-white/80 dark:bg-gray-900/80 backdrop-blur-sm z-10">
    ...
    <div class="max-w-4xl mx-auto px-6 py-8">
      ...
      <!-- Error State -->
      <div v-else-if="error" class="text-center text-red-500 bg-red-100 dark:bg-red-900/30 dark:text-red-300 p-4 rounded-lg">
        <p>Failed to get search results: {{ error }}</p>
        <button @click="performSearch(currentPage)" class="mt-2 text-sm bg-blue-600 text-white px-4 py-2 rounded hover:bg-blue-700 transition-colors">
          Retry
        </button>
      </div>
      ...
      <div v-else-if="searchQuery && rawResults">
        ...
        <div class="flex justify-between items-center mb-4">
          <div v-if="rawResults.data.searchInformation" class="text-sm text-gray-600 dark:text-gray-400">
            About {{ rawResults.data.searchInformation.formattedTotalResults }} results ({{ rawResults.data.searchInformation.formattedSearchTime }} seconds)
          </div>
          <!-- Sorting feature can be added here if needed -->
        </div>

        <!-- Spelling Suggestion -->
        <div v-if="spellingSuggestion" class="text-sm text-gray-700 dark:text-gray-300 mb-4 p-3 bg-blue-50 dark:bg-gray-800 rounded-lg">
          <p>Did you mean: <a
        ...
        <div class="border-b border-gray-200 dark:border-gray-700 mb-6">
          <nav class="-mb-px flex space-x-6" aria-label="Tabs">
            <button @click="changeSearchType('web')" :class="[searchType === 'web' ? 'border-blue-500 text-blue-600 dark:text-blue-400' : 'border-transparent text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-300 hover:border-gray-300 dark:hover:border-gray-600', 'whitespace-nowrap py-3 px-1 border-b-2 font-medium text-sm transition-colors']">
              Web
            </button>
            <button @click="changeSearchType('image')" :class="[searchType === 'image' ? 'border-blue-500 text-blue-600 dark:text-blue-400' : 'border-transparent text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-300 hover:border-gray-300 dark:hover:border-gray-600', 'whitespace-nowrap py-3 px-1 border-b-2 font-medium text-sm transition-colors']">
              Images
            </button>
          </nav>
        </div>
        ...
        <div v-if="searchType === 'web' && searchResults.length > 0" class="space-y-8">
          <div v-for="(result, index) in searchResults" :key="index" class="group">
            <a :href="result.link" target="_blank" rel="noopener noreferrer">
              <h3 class="text-xl text-blue-600 dark:text-blue-400 group-hover:underline truncate" v-html="highlightText(result.title, searchQuery)"></h3>
            </a>
            <div class="flex items-center text-sm text-green-700 dark:text-green-500 mt-1 truncate">
              <img :src="`https://www.google.com/s2/favicons?sz=16&domain_url=${result.link}`" class="w-4 h-4 mr-2 flex-shrink-0" alt="Favicon" />
              <a :href="result.link" target="_blank" rel="noopener noreferrer" class="hover:underline">
                {{ formatUrl(result.displayLink) }}
              </a>
            </div>
            <p class="text-sm text-gray-600 dark:text-gray-400 leading-relaxed mt-2 line-clamp-2" v-html="highlightText(result.snippet, searchQuery)"></p>
          </div>
        </div>

        <!-- Image Results -->
        <div v-else-if="searchType === 'image' && searchResults.length > 0" class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4">
          <div v-for="(result, index) in searchResults" :key="index" class="group relative aspect-square overflow-hidden rounded-lg">
            <a :href="result.image.contextLink" target="_blank" rel="noopener noreferrer">
              <img :src="result.link" :alt="result.title" class="w-full h-full object-cover transition-transform duration-300 group-hover:scale-110" />
              <div class="absolute inset-0 bg-black bg-opacity-40 opacity-0 group-hover:opacity-100 transition-opacity duration-300"></div>
              <div class="absolute bottom-0 left-0 p-2 w-full text-white text-xs bg-gradient-to-t from-black/60 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300">
        ...
        <div v-if="totalPages > 1 && searchResults.length > 0" class="flex justify-center mt-12">
          <nav class="flex items-center space-x-2">
            <button @click="goToPage(currentPage - 1)" :disabled="currentPage === 1" class="px-3 py-2 text-gray-600 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-700 rounded-md disabled:opacity-50 disabled:cursor-not-allowed transition-colors">
              Previous
            </button>
            <button v-for="page in paginationPages" :key="page" @click="goToPage(page)" :class="page === currentPage ? 'bg-blue-600 text-white' : 'text-gray-600 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-700'" class="w-10 h-10 rounded-md transition-colors">
              {{ page }}
            </button>
            <button @click="goToPage(currentPage + 1)" :disabled="currentPage === totalPages" class="px-3 py-2 text-gray-600 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-700 rounded-md disabled:opacity-50 disabled:cursor-not-allowed transition-colors">
              Next
            </button>
          </nav>
        </div>
